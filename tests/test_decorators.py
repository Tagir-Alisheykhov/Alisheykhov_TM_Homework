...
@log(filename="")
def func_for_test_1(digit1: int, digit2: int) -> int | float:
    """Функция для теста 1"""
    return digit1 + digit2


@log(filename="my_log.txt")
def func_for_test_2(digit1: int, digit2: int) -> int | float:
    """Функция для теста 2"""
    return digit1 + digit2


def test_my_func_output_1(capsys: Any) -> Any:
    """Захват вывода в консоль. Вариант 1"""
    func_for_test_1(1, 3)
    out, err = capsys.readouterr()
    assert out == (
        "Декоратор успешно запущен.\n"
...
def test_my_func_output_2(capsys: Any) -> Any:
